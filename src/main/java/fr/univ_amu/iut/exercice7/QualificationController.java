...
public class QualificationController {
  ...
  private void initialize() {
    ...
    // possibles :
    // "OK", "Douteux", "À jeter".
    String verdicts[] = {"OK", "Douteux", "À jeter"};
    choiceBoxVerdict.setItems(FXCollections.observableArrayList(verdicts));
    ...
    labelVerdictGlobal
        .textProperty()
        .bind(
            Bindings.when(choiceBoxVerdict.valueProperty().isNotNull())
                .then(Bindings.concat("Verdict global : ", choiceBoxVerdict.valueProperty()))
                .otherwise("Verdict global : (à saisir)"));

    // TODO exercice 7 (étape 6) : lier la TextArea de commentaire au modèle
    // (binding
    // bidirectionnel).
    nuit.commentaireProperty().bindBidirectional(zoneCommentaire.textProperty());
  }
  ...
  private void enregistrerVerdict() {
    ...
    // sélectionné.
    if (choiceBoxVerdict.getValue() != null) {
