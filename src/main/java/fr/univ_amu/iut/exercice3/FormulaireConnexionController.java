...
public class FormulaireConnexionController {
    ...
    BooleanBinding motDePasseInvalide =
        new BooleanBinding() {
          ...
          protected boolean computeValue() {
            String texte = champMotDePasse.getText();
            return texte.length() < 8
                || texte.chars().noneMatch(Character::isUpperCase)
                || texte.chars().noneMatch(Character::isDigit);
          }
        };
  ...
  private void valider() {
    ...
    // caractères.
    String MdpMasque = "*".repeat(champMotDePasse.getLength());
    labelMessage.setText(champIdentifiant.getText() + " " + MdpMasque);
  }
